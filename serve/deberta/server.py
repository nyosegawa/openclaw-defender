"""
FastAPI inference server for ProtectAI DeBERTa v3 prompt-injection classifier.

Binary classifier that detects prompt injection vs. benign input.
...
# DeBERTa binary classification: 0 = benign (SAFE), 1 = injection (INJECTION)
LABEL_MAP = {0: "benign", 1: "injection"}

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger("deberta-server")
...
app = FastAPI(
    title="DeBERTa v3 Prompt Injection Server",
    version="1.0.0",
    lifespan=lifespan,
)
